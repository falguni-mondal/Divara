        ...
        <div className='w-full' id='edit-profile-page'>
            ...
            <form onSubmit={submitHandler} className='w-full mt-8 flex flex-col gap-5' id="user-dets-update-form">
                ...
                <div className="user-firstname-update-input-container w-full">
                    <span className={`name-update-preview-title uppercase text-[#1a1a1a] font-medium text-[0.75rem]`}>change- firstname</span>
                    <div className={`update-input-container w-full h-[2.5rem] text-[1.050rem] relative ${updateTrigger.firstname ? "" : "bg-zinc-200 rounded-[3px]"}`}>
                ...
                <div className="user-lastname-update-input-container w-full">
                    <span className={`name-update-preview-title uppercase text-[#1a1a1a] font-medium text-[0.75rem]`}>change- lastname</span>
                    <div className={`update-input-container w-full h-[2.5rem] text-[1.050rem] relative ${updateTrigger.lastname ? "" : "bg-zinc-200 rounded-[3px]"}`}>
                ...
                <div className="user-email-update-input-container w-full">
                    <span className={`name-update-preview-title uppercase text-[#1a1a1a] font-medium text-[0.75rem]`}>change- email</span>
                    <div className={`update-input-container w-full h-[2.5rem] text-[1.050rem] relative ${updateTrigger.email ? "" : "bg-zinc-200 rounded-[3px]"}`}>
                ...
                <div className="user-password-update-input-container w-full">
                    <span className={`name-update-preview-title uppercase text-[#1a1a1a] font-medium text-[0.75rem]`}>change- password</span>
                    <div className={`update-input-container w-full h-[2.5rem] text-[1.050rem] relative ${updateTrigger.password ? "" : "bg-zinc-200 rounded-[3px]"}`}>
                    ...
                    <p className={`text-red-700 text-[0.7rem] mt-1 ${!formErr.newPassword && "hidden"}`}>Valid password contains 8 characters including a number and a special character.</p>
                </div>

                <div className="password-input-container w-full">
                    <span className={`name-update-preview-title uppercase text-[#1a1a1a] font-medium text-[0.75rem]`}>your password</span>
                    <div className={`update-input-container w-full h-[2.5rem] text-[1.050rem] relative ${updateTrigger.password ? "" : "bg-zinc-200 rounded-[3px]"}`}>
                        <input onBlur={() => setUpdateTrigger(prev => ({ ...prev, password: false }))} onFocus={() => setUpdateTrigger(prev => ({ ...prev, password: true }))} className={`h-full outline-0 w-full border border-zinc-300 rounded-[3px] px-2 relative z-20`} placeholder='Old Password' type="text" name='password' />
                        <BiPencil className={`${updateTrigger.password ? "hidden" : "absolute right-2 top-1/2 -translate-y-1/2 z-10"}`} />
                    </div>
                    <p className={`text-red-700 text-[0.7rem] mt-1 ${!formErr.password && "hidden"}`}>Valid password contains 8 characters including a number and a special character.</p>
                </div>
                <div className="w-full mt-2" id='user-data-update-btn-container'>
                    <button type='submit' className='h-[2.5rem] w-full rounded-[3px] bg-[#1a1a1a] text-[#efefef] uppercase text-[0.75rem] font-semibold'>update</button>
                    <span onClick={() => navigate(-1, { replace: true })} className='h-[2.5rem] w-full flex justify-center items-center mt-2 rounded-[3px] bg-zinc-200 text-[#1a1a1a] uppercase text-[0.75rem] font-semibold'>cancel</span>
                </div>
